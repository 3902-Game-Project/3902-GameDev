...
internal class ProjectileFactory : IInitable {
  ...
  internal static ProjectileFactory Instance { get; } = new();
  ...
  internal IProjectile CreateBullet(Vector2 startPosition, Vector2 direction, float velocity, float lifetime, int damage) {
    return new BulletDefault(projectileTexture, startPosition, direction, velocity, lifetime, damage);
  }

  internal IProjectile CreateBomb(Vector2 startPosition, float lifetime) {
    return new BombProjectile(projectileTexture, startPosition, lifetime);
  }

  internal static IProjectile CreateBang(ABaseGun sourceGun) {
    return new BangProjectile(TextureStore.Instance.NewGuns, sourceGun);
  }

  internal static IProjectile CreateBFGShot(Vector2 position, Vector2 direction, GunStats stats) {
    return new BFGProjectile(TextureStore.Instance.NewGuns, position, direction, stats.BulletVelocity, stats.BaseDamage);
  }

  internal IProjectile CreateBossBomb(Vector2 position, Vector2 direction, int damage) {

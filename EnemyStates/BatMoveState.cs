  ...
  public class BatMoveState : IBatState {
    ...
    public void Update(GameTime gameTime) {
      ...
      if (animationTimer >= 0.2) {
        currentFrameIndex++;
        if (currentFrameIndex >= bat.CurrentSourceRectangles.Count) currentFrameIndex = 0;
      ...
      if (bat.Position.X < 0 || bat.Position.X > 800) {
        bat.Velocity.X *= -1;
        if (bat.Velocity.X > 0) bat.FacingDirection = 1;
        else if (bat.Velocity.X < 0) bat.FacingDirection = -1;
      }
